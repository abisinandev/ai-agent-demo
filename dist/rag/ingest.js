...
const rawDocs = [
    new Document({
        pageContent: "SIP stands for Systematic Investment Plan. It allows regular investment in mutual funds. " +
    ...
    new Document({
        pageContent: "SEBI (Securities and Exchange Board of India) regulates the Indian securities market. " +
    ...
    new Document({
        pageContent: "Equity mutual funds invest mainly in company stocks. " +
...
export const ingestDocs = async () => {
    ...
    console.log(`✓ Split ${rawDocs.length} docs into ${chunks.length} chunks.`);
    chunks.forEach((c, i) => console.log(`  Chunk ${i + 1} (${c.pageContent.length} chars): "${c.pageContent.slice(0, 60)}..."`));
    console.log("\nStep 2: Converting chunks to vectors...");
    const texts = chunks.map((c) => c.pageContent);
    const vectors = await embeddings.embedDocuments(texts);
    console.log(`✓ Generated ${vectors.length} vectors, each with ${vectors[0].length} dimensions.`);
